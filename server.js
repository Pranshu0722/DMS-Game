...
const scoreSchema = new mongoose.Schema(
  {
    SEmail: { type: String, required: true, ref: "Student" },
    GID: { type: String, required: true, ref: "Game" },
    HScore: { type: Number, required: true },
  },
  { timestamps: true }
);
...
app.post("/student-login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: student._id }, "your_jwt_secret_key", {
      expiresIn: "1h",
    });
...
app.post("/questions/:gid", async (req, res) => {
  ...
  if (!Question || !QDifficulty || !Options || Options.length !== 4) {
    return res
      .status(400)
      .json({
        message: "All fields are required and Options must have 4 answers.",
      });
  }

  try {
    // Find the highest QID used across all GIDs in the database
    const highestQuestion = await QuestionModel.findOne()
      .sort({ QID: -1 })
      .limit(1);
...
app.post("/submit-score", async (req, res) => {
  const { SEmail, GID, HScore } = req.body;

  if (!SEmail || !GID || HScore === undefined) {
    return res.status(400).json({ message: "All fields are required" });
  }

  try {
    // Find the existing score for the same student and game
    const existingScore = await Score.findOne({ SEmail, GID });

    if (existingScore) {
      // If the new score is higher, update it
      if (HScore > existingScore.HScore) {
        existingScore.HScore = HScore;
        await existingScore.save();
        return res.status(200).json({ message: "Score updated successfully" });
      } else {
        return res.status(200).json({ message: "New score is not higher. No update made." });
      }
    } else {
      // If no previous score exists, create a new one
      const newScore = new Score({ SEmail, GID, HScore });
      await newScore.save();
...
app.get("/student-profile", async (req, res) => {
  ...
  try {
    // Fetch the highest score for each game the student has played
    const scores = await Score.aggregate([
      { $match: { SEmail } }, // Filter by student email
      { 
        $group: {
          _id: "$GID",
          HScore: { $max: "$HScore" } // Get highest score per game
        } 
      },
      {
        $lookup: {
          from: "games", // Assuming "games" collection stores game names
          localField: "_id",
          foreignField: "GID",
          as: "gameDetails"
        }
...
app.get("/get-student-name", async (req, res) => {
  ...
  try {
    const student = await Student.findOne({ SEmail }, "SName");
    if (!student) {
      return res.status(404).json({ message: "Student not found" });
    }
    res.status(200).json({ name: student.SName });
  } catch (error) {
    console.error("Error fetching student name:", error);
    res.status(500).json({ message: "Failed to fetch student name" });
  }
});
